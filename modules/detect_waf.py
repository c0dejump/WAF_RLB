...
def detect_wafw00f(url, directory):
    """
    WAF:
    ...
    with open(directory + "/waf.txt", "r+") as waf:
        for w in waf:
            if "behind" in w:
                ...
                pass
        if detect:
            print("\n{}{}{}".format(INFO_MOD, WARNING, message))
        else:
            print(" {} This website doesn't seem use known WAF".format(LESS))
...
def req_test_false_positif(s, res, headers):
    ...
    try:
        test_one = s.get(url_send, allow_redirects=False, verify=False, timeout=10, headers=headers)
        test_two = s.get(url_send, allow_redirects=True, verify=False, timeout=10, headers=headers)
        #print(test_one.status_code, test_two.status_code) #DEBUG
        #print("Reponse test false positive: {}".format(test_two)) #DEBUG
        #print(headers)
        if test_one.status_code == test_two.status_code:
            return True, test_two
        elif test_one.status_code == 302 or test_two.status_code == 302:
            ratio = SequenceMatcher(a=test_one.url, b=test_two.url).ratio()
            if ratio > 0.9:
                return True, test_two
            else:
                return False, False
...
#@timeit
def verify_waf(s, res, headers=False, display=False):
    """
    Function verify if there is a WAF to instable website
    """
    #360
    res_test, req_test = req_test_false_positif(s, res, headers)
    if res_test:
        ...
        "Access denied due to a large number of requests" in req_response or "has detected an attack" in req_response or "Your IP has been banned" in req_response and not forced:
            if req_test.status_code == 401 or req_test.status_code == 403 or req_test.status_code == 430:
                if display:
                    print("{}{}{} Unknown page WAF detected : {} ".format(INFO_MOD, WAF, req_test.status_code, res))
                return True
        else:
            for h in headers:
                print("{}[{}] [{} b] Potential bypass WAF rate limit with option:\033[36m -H \"{}:{}\" \033[0m".format(WAF, req_test.status_code, len(req_test.text), h, headers[h]))
